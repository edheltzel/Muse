...
`;async function fd(){try{return await Du("plugins/Muse/skills/do-muse/templates/interactive-plan-shell.html","utf8")}catch{return ku}}function pu(s){if(s.manifest.kind!=="styleguide")return"";let a=['<button type="button" data-component-filter="" aria-pressed="true">All</button>',...Array.from(new Set(Ls.map((c)=>ei[c].category))).map((c)=>`<button type="button" data-component-filter="${X(c)}" aria-pressed="false">${X(c)}</button>`)].join(""),d=s.plan.blocks.length,e=new Set(s.plan.blocks.map((c)=>c.type)).size,m=`${d} ${d===1?"example":"examples"} \xB7 ${e} unique of canonical ${Ls.length}`;return`<section class="ve-ip-explorer" data-component-explorer data-component-example-count="${d}" data-component-count="${e}" data-component-canonical-count="${Ls.length}" aria-labelledby="component-explorer-title"><div class="ve-ip-explorer-intro"><div><p class="ve-ip-label">Component reference</p><h2 id="component-explorer-title">Catalog</h2><p>Browse ${m}. Search by component name or purpose, filter by family, inspect the rendered result, then copy the exact MDX source.</p></div><label class="ve-ip-search"><span>Search components</span><input type="search" data-component-search placeholder="Try \u201Cdiagram\u201D, \u201Crisk\u201D, or \u201Creview\u201D" autocomplete="off" /></label></div><div class="ve-ip-filter-row" role="group" aria-label="Component families">${a}</div><p class="ve-ip-results" data-component-results aria-live="polite">${m}</p></section>`}function _u(s){return s.plan.blocks.map((i)=>{let a=s.manifest.kind==="styleguide"?i.type:i.props.title??i.type;return`<a href="#${X(i.id)}">${X(a)}</a>`}).join("")}async function Tu(){return`<details class="ve-ip-block ve-ip-card ve-ip-third-party-notices"><summary>Third-party font notices</summary><div class="ve-ip-third-party-notices__body"><p>Copyright notices and SIL Open Font License 1.1 terms for fonts embedded in this portable file.</p>${(await Na()).map((a)=>{let d=a.assets.map((e)=>`<li><code>${X(e)}</code></li>`).join("");return`<article><h3>${X(a.package)} ${X(a.version)}</h3><p>Embedded assets:</p><ul>${d}</ul><pre class="code-block">${X(a.text)}</pre></article>`}).join("")}</div></details>`}async function ud(s,i=!1,a,d){let e=a??await fd(),m=await sr(d),c=Gu.replaceAll("{{PRIMARY}}",m.primary),v=s.manifest.kind==="styleguide",g=[Si(s.plan.blocks,{staticMode:i,componentExplorer:v}),s.canvas?`<section class="ve-ip-block ve-ip-card" id="canvas"><div class="ve-ip-label">Canvas</div><h2>Canvas</h2>${Si(s.canvas.blocks,{staticMode:i,componentExplorer:!1})}</section>`:"",i?await Tu():""].join(`
`),t=await Ma(i),y=i?"":'<section class="ve-ip-review-sync" data-review-sync aria-live="polite"><strong data-review-sync-title>Loading saved review\u2026</strong><span data-review-sync-detail>Review controls unlock after server state and comments load.</span><button type="button" data-review-retry hidden>Retry</button></section>',x=e.replaceAll("{{TITLE}}",X(s.manifest.title)).replaceAll("{{KIND}}",X(s.manifest.kind)).replaceAll("{{SUBTITLE}}",v?"A human-facing reference for every renderer-owned MDX component.":i?"Static export. Interactive persistence requires the local review bridge.":"Local interactive review surface.").replaceAll("{{REVIEW_AUTHORITY}}",i?"static":"loading").replaceAll("{{EXPLORER}}",pu(s)).replaceAll("{{REVIEW_SYNC}}",y).replaceAll("{{NAV}}",_u(s)).replaceAll("{{CONTENT}}",g).replaceAll("{{FONTS}}",t).replaceAll("{{STYLE}}",c).replaceAll("{{MERMAID_URL}}",Oa).replaceAll("{{MERMAID_SRI}}",na).replaceAll("{{CLIENT}}",i?Ea:Ia),b=[...s.plan.blocks,...s.canvas?.blocks??[]],Q=v?new Set(s.plan.blocks):void 0,z=b.map(({id:Z})=>Z);if(s.canvas)z.push("canvas");for(let Z of b){if(Z.type!=="CommentAnchor"||Q?.has(Z))z.push(..._(Z,"title"));if(z.push(..._(Z,"instructions")),i)continue;z.push(..._(Z,"tabs"),..._(Z,"panels"))}let V=pr(x,z);if(V.length)throw Error(`Invalid rendered HTML:
${V.join(`
`)}`);return x}async function zi(s,i){let a=await ed(s),d=la(s,a.manifest.dist);await Iu(d,{recursive:!0}),await Sa(d);let e=la(d,"index.html"),m=la(d,"static-export.html"),c=await fd();return await md(e,await ud(a,!1,c,i)),await md(m,await ud(a,!0,c,i)),{indexPath:e,staticExportPath:m}}import{readFile as $d,realpath as Bf}from"fs/promises";import{join as Ed}from"path";import{createHash as zf,randomUUID as cs}from"crypto";import{constants as G,realpathSync as Zf}from"fs";import{link as Yf,lstat as q,mkdir as Va,open as Li,readdir as As,readlink as za,realpath as Qs,rename as ls,rm as T,symlink as Bi,writeFile as Yd}from"fs/promises";import{basename as Jf,join as Y,resolve as js}from"path";import{isDeepStrictEqual as ji}from"util";import{join as vd}from"path";var Au=/[\\\u0000-\u001f\u007f-\u00a0<>&#*_`\[\]()!|]/g;function vs(s){return String(s??"").replace(Au,(i)=>`\\u${i.charCodeAt(0).toString(16).padStart(4,"0")}`)}function sf(s){let i={};for(let a of s.plan.blocks){let d=k(a.body);if(d.length)(i[a.type]??=[]).push(...d)}return i}function ga(s,i){let a=Tr(i);if(a.length)throw Error(a.join(`
...
`));return a}function ta(s){let i=(e)=>e.length?e.map((m)=>`- ${vs(m)}`).join(`
...
${i(s.openRisks)}
`}import{constants as ws}from"fs";import{lstat as qs,open as hd,readFile as af,realpath as rf}from"fs/promises";import{resolve as Zi}from"path";import{dlopen as wa,FFIType as o,ptr as ld,read as df}from"bun:ffi";var xa=".muse-review.lock",ef=2,mf=4,uf=8,Yi=500,ff=3221225472,vf=3,cf=4,lf=128,gf=2097152,tf=3,bf=32,wf=33,xf=0xffffffffffffffffn,yf,gd={flock:{args:[o.i32,o.i32],returns:o.i32}};function td(s,i){if(s===null)throw Error(`${i} returned a null errno pointer`);return df.i32(s)}function ba(s,i){if(i==="darwin"){let d=wa(s,{...gd,__error:{args:[],returns:o.ptr}});return{flock:(e,m)=>d.symbols.flock(e,m),errno:()=>td(d.symbols.__error(),"__error")}}let a=wa(s,{...gd,__errno_location:{args:[],returns:o.ptr}});return{flock:(d,e)=>a.symbols.flock(d,e),errno:()=>td(a.symbols.__errno_location(),"__errno_location")}}function hf(s,i){return(i==="darwin"?{5:"EIO",9:"EBADF",35:"EWOULDBLOCK",45:"ENOTSUP",77:"ENOLCK",102:"EOPNOTSUPP"}:{5:"EIO",9:"EBADF",11:"EWOULDBLOCK",37:"ENOLCK",38:"ENOSYS",95:"EOPNOTSUPP"})[s]??`ERRNO_${s}`}function bd(s,i,a,d){let e=hf(a,d),m=Object.assign(Error(`${e}: flock ${i} failed for ${s}`),{code:e,errno:a,syscall:"flock",path:s});return Error(`Could not ${i} review lock at ${s}: ${e} (${a})`,{cause:m})}function wd(s,i,a,d,e){if(s(a,ef|mf)!==0){let m=i();if(m===(e==="darwin"?35:11))return;throw bd(d,"acquire",m,e)}return{release(){if(s(a,uf)!==0)throw bd(d,"release",i(),e)}}}function Hf(s){let i=new Uint16Array(s.length+1);for(let a=0;a<s.length;a+=1)i[a]=s.charCodeAt(a);return i}async function Ff(){if(process.platform==="win32"){let d=wa("kernel32.dll",{CreateFileW:{args:[o.ptr,o.u32,o.u32,o.ptr,o.u32,o.u32,o.u64],returns:o.u64},LockFileEx:{args:[o.u64,o.u32,o.u32,o.u32,o.u32,o.ptr],returns:o.i32},CloseHandle:{args:[o.u64],returns:o.i32},GetLastError:{args:[],returns:o.u32}});return{kind:"windows",async tryLock(e){let m=Zi(e.path,xa),c=Hf(m),v=d.symbols.CreateFileW(ld(c),ff,vf,null,cf,lf|gf,0n);if(v===xf){let y=d.symbols.GetLastError();if(y===bf)return;throw Error(`CreateFileW failed for the review lock with Windows error ${y}`)}let g;try{if(g=await qs(m,{bigint:!0}),!g.isFile()||g.isSymbolicLink())throw Error(`Review lock path must be a regular non-symlink file at ${m}`)}catch(y){throw d.symbols.CloseHandle(v),y}let t=new Uint8Array(32);for(let y=0;y<Yi;y+=1){if(d.symbols.LockFileEx(v,tf,0,4294967295,4294967295,ld(t))!==0){try{let b=await qs(m,{bigint:!0});if(!b.isFile()||b.isSymbolicLink()||b.dev!==g.dev||b.ino!==g.ino||b.size!==g.size||b.mtimeNs!==g.mtimeNs||b.ctimeNs!==g.ctimeNs)throw Error(`Review lock path generation changed during Windows acquisition at ${m}`)}catch(b){throw d.symbols.CloseHandle(v),b}return{release(){d.symbols.CloseHandle(v)}}}let x=d.symbols.GetLastError();if(x!==wf)throw d.symbols.CloseHandle(v),Error(`LockFileEx failed for the review lock with Windows error ${x}`);if(y+1===Yi)throw d.symbols.CloseHandle(v),Error(`Timed out waiting for Windows review lock at ${m}`);await Bun.sleep(10)}throw d.symbols.CloseHandle(v),Error(`Timed out waiting for Windows review lock at ${m}`)}}}if(process.platform!=="darwin"&&process.platform!=="linux")throw Error(`Review locking is unsupported on ${process.platform}`);let s=process.platform,i;if(s==="darwin")i=ba("/usr/lib/libSystem.B.dylib",s);else{let d=process.arch==="x64"?"x86_64":process.arch==="arm64"?"aarch64":process.arch,e=["libc.so.6","libc.so",`ld-musl-${d}.so.1`,`/lib/ld-musl-${d}.so.1`];for(let m of e)try{i=ba(m,s);break}catch{}if(!i){let m="";try{m=await af("/proc/self/maps","utf8")}catch{throw Error("Could not resolve Linux libc for review locking")}let v=(m.match(/\/\S*(?:libc\.so(?:\.\d+)*|ld-musl-[^\s/]+\.so\.1)/g)??[]).find((g)=>!g.includes("libcap")&&!g.includes("libcrypto"));if(!v)throw Error("Could not resolve the loaded Linux libc for review locking");i=ba(v,s)}}if(!i)throw Error("Could not initialize Unix libc review locking");let a=i;return{kind:"unix",async tryLock(d){return wd(a.flock,a.errno,d.handle.fd,d.path,s)},async tryLegacyLock(d){return wd(a.flock,a.errno,d.handle.fd,d.path,s)}}}async function Qf(s){let i=await rf(s),a=await hd(i,ws.O_RDONLY|ws.O_DIRECTORY|ws.O_NOFOLLOW|ws.O_NONBLOCK);try{let[d,e]=await Promise.all([a.stat({bigint:!0}),qs(i,{bigint:!0})]);if(!d.isDirectory()||!e.isDirectory()||e.isSymbolicLink()||d.dev!==e.dev||d.ino!==e.ino)throw Error(`Plan root is not bound to its opened generation at ${i}`);return{path:i,handle:a,descriptor:d}}catch(d){throw await a.close(),d}}async function xd(s){let[i,a]=await Promise.all([s.handle.stat({bigint:!0}),qs(s.path,{bigint:!0})]);if(!i.isDirectory()||!a.isDirectory()||a.isSymbolicLink()||i.dev!==s.descriptor.dev||i.ino!==s.descriptor.ino||a.dev!==i.dev||a.ino!==i.ino)throw Error(`Plan root generation changed at ${s.path}`)}async function Xf(s){let i=Zi(s.path,xa),a=await hd(i,ws.O_RDWR|ws.O_CREAT|ws.O_NOFOLLOW|ws.O_NONBLOCK,384);try{let[d,e]=await Promise.all([a.stat({bigint:!0}),qs(i,{bigint:!0})]);if(!d.isFile()||!e.isFile()||e.isSymbolicLink()||d.dev!==e.dev||d.ino!==e.ino)throw Error(`Review lock path is not bound to its opened file at ${i}`);return{path:i,handle:a,descriptor:d}}catch(d){throw await a.close(),d}}async function Vf(s){let[i,a]=await Promise.all([s.handle.stat({bigint:!0}),qs(s.path,{bigint:!0})]);if(!i.isFile()||!a.isFile()||a.isSymbolicLink()||i.dev!==s.descriptor.dev||i.ino!==s.descriptor.ino||a.dev!==i.dev||a.ino!==i.ino)throw Error(`Review lock path generation changed at ${s.path}`)}async function yd(s,i,a,d){let e=[];try{a?.release()}catch(m){e.push(m)}try{await d?.handle.close()}catch(m){e.push(m)}try{s.release()}catch(m){e.push(m)}try{await i.handle.close()}catch(m){e.push(m)}return e}async function ya(s){let i=await(yf??=Ff());for(let a=0;a<Yi;a+=1){let d=await Qf(s),e;try{e=await i.tryLock(d)}catch(v){throw await d.handle.close(),v}if(e===void 0){await d.handle.close(),await Bun.sleep(10);continue}let m,c;try{if(i.kind==="unix"){m=await Xf(d);for(let y=0;y<Yi;y+=1){if(await xd(d),c=await i.tryLegacyLock(m),c!==void 0)break;await Bun.sleep(10)}if(!c)throw Error(`Timed out waiting for legacy review lock at ${Zi(d.path,xa)}`)}let v=!1,g=async()=>{if(v)throw Error(`Review lock was already released for ${d.path}`);await xd(d)},t=async()=>{if(await g(),m)await Vf(m)};return await t(),{assertOwned:t,assertCanonicalOwned:g,async release(){if(v)return;v=!0;let y=await yd(e,d,c,m);if(y.length)throw AggregateError(y,`Review lock release failed for ${d.path}`)}}}catch(v){let g=await yd(e,d,c,m);if(g.length)throw AggregateError([v,...g],`Review lock acquisition cleanup failed for ${d.path}`);throw v}}throw Error(`Timed out waiting for review lock for ${Zi(s)}`)}var Ki=".muse-review",p="current",Jd="initialized",Xs=["plan-state.json","comments.json","agent-handoff.json","agent-handoff.md"],qf=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/,qd=64;class Pi extends Error{constructor(){super(`Review wait limit exceeded; at most ${qd} waiters are supported`);this.name="ReviewWaiterLimitError"}}class Ha extends Error{constructor(){super("Review wait aborted");this.name="ReviewWaitAbortedError"}}var qi=new Map,Ts=new Map;function Ji(s){try{s?.()}catch{}}function Wf(s,i){let a={planDir:s,generation:i};for(let e of qi.get(s)??[])try{e(a)}catch{}let d=Ts.get(s);if(!d)return;Ts.delete(s);for(let e of d)e.resolve(i)}function Wd(s,i){let a=Zf(s),d=qi.get(a)??new Set;return d.add(i),qi.set(a,d),()=>{if(d.delete(i),d.size===0)qi.delete(a)}}async function jd(s,i,a={}){let d=await Qs(s),e=Ts.get(d)??new Set;if(e.size>=qd)throw new Pi;let m=!1,c=!1,v,g=(x)=>{if(e.delete(x),e.size===0)Ts.delete(d);if(v)a.signal?.removeEventListener("abort",v)},t,y=new Promise((x,b)=>{t={resolve:x,reject:b}});if(v=()=>{if(g(t),t.reject(new Ha),m&&!c)c=!0,Ji(a.onCancelled)},a.signal?.aborted)throw v(),new Ha;a.signal?.addEventListener("abort",v,{once:!0}),e.add(t),Ts.set(d,e);try{let x=await ii(d);if(x.generation!==i)return g(t),x;return m=!0,Ji(a.onParked),await y,g(t),c=!0,Ji(a.onDelivered),await ii(d)}catch(x){if(g(t),m&&!c)c=!0,Ji(a.onCancelled);throw x}}class gs extends Error{failure;constructor(s,i){super(i);this.failure=s;this.name="ReviewOperationError"}}class Za extends Error{published;restorable;constructor(s,i,a){super("Published review snapshot failed canonical postcommit verification",{cause:a});this.published=s;this.restorable=i;this.name="SnapshotCommitVerificationError"}}function L(s,i){return s.dev===i.dev&&s.ino===i.ino&&s.size===i.size&&s.mtimeNs===i.mtimeNs&&s.ctimeNs===i.ctimeNs}function Ws(s,i){return s.dev===i.dev&&s.ino===i.ino}async function A(s){let i=await Li(s,G.O_RDONLY|G.O_DIRECTORY|G.O_NOFOLLOW|G.O_NONBLOCK);try{let[a,d]=await Promise.all([i.stat({bigint:!0}),q(s,{bigint:!0})]);if(!a.isDirectory()||!d.isDirectory()||d.isSymbolicLink()||a.dev!==d.dev||a.ino!==d.ino)throw Error(`Directory path is not bound to its opened generation at ${s}`);return{path:s,handle:i,stats:a}}catch(a){throw await i.close(),a}}async function U(s){let[i,a]=await Promise.all([s.handle.stat({bigint:!0}),q(s.path,{bigint:!0})]);if(i.dev!==s.stats.dev||i.ino!==s.stats.ino||a.dev!==s.stats.dev||a.ino!==s.stats.ino)throw Error(`Directory path generation changed at ${s.path}`)}async function Ya(s,i){if(i>BigInt(Number.MAX_SAFE_INTEGER))throw Error("Plan authority file is too large to read safely");let a=Buffer.alloc(Number(i)),d=0;while(d<a.length){let e=await s.read(a,d,a.length-d,d);if(e.bytesRead===0)throw Error("Plan authority file ended while being read");d+=e.bytesRead}return a}async function ha(s,i){let a;try{a=await Li(s,G.O_RDONLY|G.O_NOFOLLOW|G.O_NONBLOCK)}catch(d){if(!i&&typeof d==="object"&&d!==null&&"code"in d&&d.code==="ENOENT")return{path:s,exists:!1};throw d}try{let[d,e]=await Promise.all([a.stat({bigint:!0}),q(s,{bigint:!0})]);if(!d.isFile()||!e.isFile()||e.isSymbolicLink()||d.dev!==e.dev||d.ino!==e.ino)throw Error(`Plan authority path is not bound to its opened file at ${s}`);let m=await Ya(a,d.size),[c,v]=await Promise.all([a.stat({bigint:!0}),q(s,{bigint:!0})]);if(!L(d,c)||!L(d,v))throw Error(`Plan authority changed while being captured at ${s}`);return{path:s,exists:!0,bytes:m,handle:a,stats:d}}catch(d){throw await a.close(),d}}async function Kd(s){let i=await q(s,{bigint:!0});if(!i.isDirectory()||i.isSymbolicLink())throw Error(`Plan root must be a non-symlink directory at ${s}`);let a=await Promise.allSettled([ha(Y(s,"plan.mdx"),!0),ha(Y(s,"canvas.mdx"),!1),ha(Y(s,"visual-explainer.json"),!1)]),d=a.find((m)=>m.status==="rejected");if(d)throw await Promise.allSettled(a.filter((m)=>m.status==="fulfilled").map((m)=>m.value.handle?.close())),d.reason;let e=a.map((m)=>m.value);try{let m=await q(s,{bigint:!0});if(!L(i,m))throw Error(`Plan root changed while authority was captured at ${s}`);let[c,v,g]=e;return{root:i,files:e,plan:ca(s,{plan:c.bytes.toString("utf8"),canvas:v.bytes?.toString("utf8"),manifest:g.bytes?.toString("utf8")})}}catch(m){throw await Promise.allSettled(e.map((c)=>c.handle?.close())),m}}async function Ja(s,i){let a=await q(s,{bigint:!0});if(i.root.dev!==a.dev||i.root.ino!==a.ino)throw Error(`Plan root generation changed at ${s}`);for(let d of i.files){if(!d.exists){if(await od(d.path))throw Error(`Plan source appeared during approval at ${d.path}`);continue}let[e,m,c]=await Promise.all([d.handle.stat({bigint:!0}),q(d.path,{bigint:!0}),Ya(d.handle,d.stats.size)]);if(!L(d.stats,e)||!L(d.stats,m)||!c.equals(d.bytes))throw Error(`Plan source changed during approval at ${d.path}`)}}async function Ud(s){if(s)await Promise.allSettled(s.files.map((i)=>i.handle?.close()))}async function rs(s,i){let a=Y(s,Ki);if(i)try{await Va(a)}catch(c){if(!(typeof c==="object"&&c!==null&&("code"in c)&&c.code==="EEXIST"))throw c}let d=await q(a);if(!d.isDirectory()||d.isSymbolicLink())throw Error(`Review store must be a plan-local non-symlink directory at ${a}`);let[e,m]=await Promise.all([Qs(s),Qs(a)]);if(m!==js(e,Ki))throw Error(`Review store escapes the plan directory at ${a}`);return a}async function Ci(s,i){let a=await rs(s,i),d=Y(a,"bundles");if(i)try{await Va(d)}catch(v){if(!(typeof v==="object"&&v!==null&&("code"in v)&&v.code==="EEXIST"))throw v}let e=await q(d);if(!e.isDirectory()||e.isSymbolicLink())throw Error(`Review bundles root must be a plan-local non-symlink directory at ${d}`);let[m,c]=await Promise.all([Qs(a),Qs(d)]);if(c!==js(m,"bundles"))throw Error(`Review bundles root escapes the review store at ${d}`);return d}async function Ks(s,i){let a=await rs(s,!1),d=await Ci(s,!1),e=await q(i,{bigint:!0});if(!e.isSymbolicLink())throw Error(`Current review pointer is not a symlink at ${i}`);let m=await za(i),c=m.replaceAll("\\","/"),v=c.match(/^bundles\/([0-9a-f-]+)$/);if(!v||!qf.test(v[1]))throw Error(`Invalid current review bundle target '${m}'`);let g=Y(d,v[1]),[t,y]=await Promise.all([q(g,{bigint:!0}),q(i,{bigint:!0})]);if(!L(e,y))throw Error(`Current review pointer generation changed at ${i}`);if(!t.isDirectory()||t.isSymbolicLink()||js(a,c)!==js(g))throw Error(`Current review bundle is not a regular directory at ${g}`);return{id:v[1],path:g,directory:t,pointer:e}}async function Ui(s){let i=await rs(s,!1);return Ks(s,Y(i,p))}async function Rd(s){let i;try{i=await rs(s,!1)}catch(a){if(typeof a==="object"&&a!==null&&"code"in a&&a.code==="ENOENT")return;throw a}try{await q(Y(i,p))}catch(a){if(typeof a==="object"&&a!==null&&"code"in a&&a.code==="ENOENT")return;throw a}return Ui(s)}async function od(s){try{return await q(s),!0}catch(i){if(typeof i==="object"&&i!==null&&"code"in i&&i.code==="ENOENT")return!1;throw i}}function Hd(s){return typeof s==="string"&&Number.isFinite(Date.parse(s))&&new Date(s).toISOString()===s}function Ld(s){if(s===void 0)return[];let i=JSON.parse(s);if(!Array.isArray(i))throw Error("Comments must be an array");let a={id:!0,blockId:!0,anchor:!0,body:!0,status:!0,createdAt:!0,resolvedAt:!0},d=new Set;for(let e of i){if(!e||typeof e!=="object"||Array.isArray(e))throw Error("Each comment must be an object");let m=e,c=Object.keys(m).find((g)=>!a[g]);if(c)throw Error(`Comment contains unknown field '${c}'`);let v=m.status==="resolved";if(typeof m.id!=="string"||m.id.trim().length===0||d.has(m.id)||typeof m.blockId!=="string"||m.blockId.trim().length===0||typeof m.body!=="string"||m.body.trim().length===0||!Hd(m.createdAt)||m.status!=="open"&&!v||m.anchor!==void 0&&(typeof m.anchor!=="string"||m.anchor.trim().length===0)||v&&(!Hd(m.resolvedAt)||Date.parse(m.resolvedAt)<Date.parse(m.createdAt))||!v&&m.resolvedAt!==void 0)throw Error("Comments must have unique nonblank ids, valid timestamps, and coherent open/resolved fields");d.add(m.id)}return i}function Bd(s,i){return[...s.plan.blocks,...s.canvas?.blocks??[]].some((a)=>a.id===i)}function qa(s,i){let a=i.find((d)=>!Bd(s,d.blockId));if(a)throw Error(`Persisted comment '${a.id}' references unknown blockId '${a.blockId}'`)}async function ys(s,i,a){let d=await Pd(s,a);return qa(i.plan,d.comments),d}function Fa(s){if(Array.isArray(s))return s.map(Fa);if(!s||typeof s!=="object")return s;return Object.fromEntries(Object.entries(s).sort(([i],[a])=>i.localeCompare(a)).map(([i,a])=>[i,Fa(a)]))}function si(s,i,a){let{approvalDigest:d,...e}=i,m=Fa({sources:Object.fromEntries(s.files.map((c)=>[Jf(c.path),c.exists?{exists:!0,bytes:c.bytes.toString("base64")}:{exists:!1}])),state:e,comments:a});return zf("sha256").update(JSON.stringify(m)).digest("hex")}function Wa(s,i){if(i.state.status!=="approved"||i.handoffJson===void 0||i.handoffMarkdown===void 0||i.state.approvalDigest!==si(s,i.state,i.comments))throw Error("No coherent approved handoff is published");let a=JSON.parse(i.handoffJson),d=ga(s.plan,i.state);if(ps(a).length>0||!ji(a,d)||i.handoffMarkdown!==ta(d)||!ji(cd(i.handoffMarkdown),d))throw Error("No coherent approved handoff is published")}async function jf(s,i){let a=Y(s,i),d;try{d=await Li(a,G.O_RDONLY|G.O_NOFOLLOW|G.O_NONBLOCK)}catch(c){if(typeof c==="object"&&c!==null&&"code"in c&&c.code==="ELOOP")throw Error(`Review bundle member '${i}' must be a regular non-symlink file`);throw c}let[e,m]=await Promise.all([d.stat({bigint:!0}),q(a,{bigint:!0})]);if(!e.isFile()||!m.isFile()||m.isSymbolicLink()||e.dev!==m.dev||e.ino!==m.ino)throw await d.close(),Error(`Review bundle member '${i}' must be a stable regular non-symlink file`);return{file:i,path:a,handle:d,before:e}}async function Pd(s,i){let a=await q(s,{bigint:!0});if(i&&!L(i,a))throw Error(`Review bundle path generation changed before it was read at ${s}`);let d=(await As(s)).sort();for(let x of d)if(!Xs.includes(x))throw Error(`Review bundle contains unexpected member '${x}' at ${s}`);for(let x of["plan-state.json","comments.json"])if(!d.includes(x))throw Error(`Review bundle is missing ${x} at ${s}`);if(["agent-handoff.json","agent-handoff.md"].filter((x)=>d.includes(x)).length===1)throw Error(`Review bundle must contain a coherent handoff pair at ${s}`);let m={},c=[];try{for(let x of d)c.push(await jf(s,x));for(let x of c)m[x.file]=await x.handle.readFile("utf8");for(let x of c){let[b,Q]=await Promise.all([x.handle.stat({bigint:!0}),q(x.path,{bigint:!0})]);if(!L(b,x.before)||!L(Q,x.before))throw Error(`Review bundle member '${x.file}' changed or was rebound while it was being read at ${s}`)}}finally{await Promise.allSettled(c.map((x)=>x.handle.close()))}let[v,g]=await Promise.all([As(s).then((x)=>x.sort()),q(s,{bigint:!0})]);if(!ji(v,d)||!L(g,a)||i!==void 0&&!L(g,i))throw Error(`Review bundle changed or was rebound while it was being read at ${s}`);let t=JSON.parse(m["plan-state.json"]),y=Js(t);if(y.length)throw Error(y.join(`
`));return{state:t,comments:Ld(m["comments.json"]),handoffJson:m["agent-handoff.json"],handoffMarkdown:m["agent-handoff.md"]}}async function Fd(s,i){qa(i.plan,s.comments);let a=i.plan,d=s.comments.filter((y)=>y.status==="open").map((y)=>y.id),e={...s.state,unresolvedCommentIds:d},{handoffJson:m,handoffMarkdown:c}=s,v=!1,g=d.length===0&&ma([...a.plan.blocks,...a.canvas?.blocks??[]],e).length===0,t=e.status!=="approved"||e.approvalDigest===si(i,e,s.comments);if(e.status==="approved"&&g&&t)try{Wa(i,{...s,state:e}),v=!0}catch{v=!1}if(e.status==="approved"&&(!g||!t||!v)){let{approvedAt:y,reviewer:x,approvalDigest:b,...Q}=e;e={...Q,status:"needs_revision"}}if(e.status!=="approved")m=void 0,c=void 0;return{state:e,comments:s.comments,handoffJson:m,handoffMarkdown:c}}function Kf(s,i){if(i==="plan-state.json")return`${JSON.stringify(s.state,null,2)}
`;if(i==="comments.json")return`${JSON.stringify(s.comments,null,2)}
`;if(i==="agent-handoff.json")return s.handoffJson;return s.handoffMarkdown}async function Ri(s,i,a,d){let e=`${i}.${cs()}.invalid`;await U(s),await d(),await ls(i,e);let m=await n(e),c,v;try{c=await Ks(js(s.path,".."),e)}catch(g){v=g}if(m.stats&&Ws(m.stats,a.pointer)){if(v!==void 0)throw AggregateError([v],"Moved expected review pointer could not be verified");if(!c||c.id!==a.id||!L(c.directory,a.directory))throw Error("Moved expected review pointer resolved a different publication");return!0}try{if(m.kind!=="symlink")throw Error(`Moved review pointer is not a symlink at ${e}`);await U(s),await d(),await Bi(m.target,i);let g=await Ks(js(s.path,".."),i);if(c&&(g.id!==c.id||!L(g.directory,c.directory)))throw Error("Restored review successor does not resolve the moved publication");let t={path:i,kind:"symlink",target:m.target,stats:g.pointer};await d(),await Promise.all([Xa(t),ja(m)]),await T(e,{force:!0})}catch(g){throw AggregateError([...v===void 0?[]:[v],g],c?"Distinct review successor could not be restored after conditional quarantine":"Moved review pointer could not be verified and restoration was incomplete")}if(v!==void 0)throw v;return!1}async function xs(s,i,a,d,e,m=!1){let c=await rs(s,!0),v=await Ci(s,!0),[g,t]=await Promise.all([A(c),A(v)]),y=cs(),x=Y(v,`${y}.staging`),b=Y(v,y),Q=Y(c,`current.${y}.pointer`),z=!1,V,Z,W=async()=>{await Promise.allSettled([V?.handle.close(),t.handle.close(),g.handle.close()])},I=async()=>{let D=!0;try{if(await Promise.all([U(g),U(t)]),z){let ts=await q(b,{bigint:!0});D=ts.dev===V.stats.dev&&ts.ino===V.stats.ino}}catch{D=!1}if(D)await a(),await Promise.allSettled([T(x,{recursive:!0,force:!0}),T(Q,{force:!0}),...z?[T(b,{recursive:!0,force:!0})]:[]])};try{await U(t),await a(),await Va(x),V=await A(x);for(let D of Xs){let ts=Kf(i,D);if(ts!==void 0)await U(V),await a(),await Yd(Y(x,D),ts,{flag:"wx"})}await U(V),await U(t),await a(),await ls(x,b),z=!0,Z=await q(b,{bigint:!0}),await U(g),await a(),await Bi(`bundles/${y}`,Q),await e?.(),await U(g)}catch(D){throw await I(),await W(),D}if(!Z)throw await W(),Error("Published review bundle generation was not captured");let B=await Ks(s,Q),j={id:y,path:b,directory:Z,pointer:B.pointer};if(B.id!==j.id||!L(B.directory,j.directory))throw await I(),await W(),Error("Prepared current pointer does not resolve the published review generation");await Promise.allSettled([V?.handle.close(),t.handle.close()]),await a(),await ls(Q,Y(c,p)),d?.(j);let ms=!1,Ca=!1;try{await U(g),ms=!0;let D=await Ui(s);if(Ca=!0,D.id!==j.id||!L(D.directory,j.directory)||!Ws(D.pointer,j.pointer))throw Error("Canonical current pointer does not resolve the published review generation")}catch(D){let ts=ms&&m&&!Ca;if(ms&&!ts)await Ri(g,Y(c,p),j,a);throw new Za(j,ts,D)}finally{await g.handle.close().catch(()=>{return})}return j}async function Qa(s,i,a,d,e){let m=await rs(s,!1),c=await A(m),v=Y(m,`current.${cs()}.rollback`),g,t=!1;try{let y=await Ui(s);if(y.id!==d.id||!L(y.directory,d.directory)||!Ws(y.pointer,d.pointer))throw Error("Published approval generation changed before rollback");await Bi(`bundles/${i.id}`,v),await U(a),await U(c),g=await q(v,{bigint:!0});let x=await q(Y(m,p),{bigint:!0});if(!Ws(x,d.pointer))throw Error("Published approval pointer generation changed before rollback");await e(),await ls(v,Y(m,p)),t=!0,await U(c);let b=await Ui(s);if(b.id!==i.id||!L(b.directory,i.directory)||!Ws(b.pointer,g))throw Error("Canonical current pointer does not resolve the restored prior generation")}catch(y){let x=[];try{if(t&&g)await Ri(c,Y(m,p),{...i,pointer:g},e);else await U(c),await Ri(c,Y(m,p),d,e)}catch(b){x.push(b)}if(!t)try{await T(v,{force:!0})}catch(b){x.push(b)}if(await Promise.allSettled([a.handle.close(),c.handle.close()]),x.length)throw AggregateError([y,...x],"Prior review publication rollback failed and cleanup was incomplete");throw y}await Promise.allSettled([a.handle.close(),c.handle.close()])}async function Uf(s,i,a,d){let e=await rs(s,!1),m=Y(e,`current.${cs()}.invalid`);if(await d(),await ls(Y(e,p),m),(await Ks(s,m)).id!==i.id)throw Error("Current approval generation changed while being invalidated");let v=await xs(s,a,d);try{await d(),await T(m,{force:!0})}catch(g){console.warn(`Invalidation committed; deferred pointer cleanup: ${g instanceof Error?g.message:String(g)}`)}return v}async function Rf(s){let i=await rs(s,!1),a=(await As(i)).filter((d)=>d.startsWith("current.")&&d.endsWith(".invalid"));if(a.length===0)return;if(a.length!==1)throw Error("Review store has multiple invalidated approval generations");return Ks(s,Y(i,a[0]))}class O extends Error{}async function n(s){let i;try{i=await q(s,{bigint:!0})}catch(d){if(typeof d==="object"&&d!==null&&"code"in d&&d.code==="ENOENT")return{path:s,kind:"missing"};throw d}if(i.isSymbolicLink()){let d=await za(s),e=await q(s,{bigint:!0});if(!L(i,e))throw new O(`Legacy path changed while captured at ${s}`);return{path:s,kind:"symlink",target:d,stats:i}}if(!i.isFile())throw Error(`Unsupported compatibility path at ${s}`);let a=await Li(s,G.O_RDONLY|G.O_NOFOLLOW|G.O_NONBLOCK);try{let d=await a.stat({bigint:!0});if(d.dev!==i.dev||d.ino!==i.ino)throw new O(`Legacy path was rebound while captured at ${s}`);let e=await Ya(a,d.size),[m,c]=await Promise.all([a.stat({bigint:!0}),q(s,{bigint:!0})]);if(!L(d,m)||!L(d,c))throw new O(`Legacy path changed while captured at ${s}`);return{path:s,kind:"file",content:e,stats:d}}finally{await a.close()}}async function Cd(s){return Promise.all(Xs.map((i)=>n(Y(s,i))))}async function Xa(s){let i=await n(s.path);if(!(i.kind===s.kind&&(i.kind==="missing"||L(i.stats,s.stats)&&(i.kind==="file"?i.content.equals(s.content):i.target===s.target))))throw new O(`Legacy path generation changed at ${s.path}`)}function Od(s){let i=(m)=>{let c=s[Xs.indexOf(m)];if(c.kind==="symlink")throw Error(`Legacy review path must not be a symlink at ${c.path}`);return c.content?.toString("utf8")},a=i("plan-state.json"),d=a===void 0?kr():JSON.parse(a),e=Js(d).filter((m)=>m!=="Approved ReviewState.approvalDigest must be a canonical SHA-256 digest");if(e.length)throw Error(e.join(`
`));return{state:d,comments:Ld(i("comments.json")),handoffJson:i("agent-handoff.json"),handoffMarkdown:i("agent-handoff.md")}}function Us(s,i){return s.kind===i.kind&&s.kind!=="missing"&&s.stats.dev===i.stats.dev&&s.stats.ino===i.stats.ino&&(s.kind==="file"?s.content.equals(i.content):s.target===i.target)}function Qd(s,i){if(s.stats!==void 0)return Us(s,i);return s.kind==="symlink"&&i.kind==="symlink"&&s.target===i.target}async function ja(s){let i=await n(s.path);if(!(s.kind==="missing"?i.kind==="missing":Us(s,i)))throw new O(`Legacy path generation changed at ${s.path}`)}async function oi(s,i,a){let d=await n(i);if(!Us(s,d))throw new O(`Quarantined compatibility generation changed at ${s.path}`);await a(),await Yf(i,s.path),await ja(s),await a(),await T(i,{force:!0})}async function Wi(s,i){let a=`${s.original.path}.${cs()}.rollback`;await i();let d=await n(s.original.path);if(!s.created||!Qd(s.installed,d))throw new O(`Compatibility path changed before rollback at ${s.original.path}`);if(s.quarantine){let m=await n(s.quarantine);if(!Us(s.original,m))throw new O(`Quarantined compatibility generation changed at ${s.original.path}`)}else if(s.original.kind!=="missing")throw Error(`Compatibility rollback is missing quarantine authority at ${s.original.path}`);await i(),await ls(s.original.path,a);let e=await n(a);if(!Qd(s.installed,e))throw new O(`Compatibility path changed during rollback at ${s.original.path}`);try{if((await n(s.original.path)).kind!=="missing")throw new O(`External compatibility successor appeared during rollback at ${s.original.path}`);if(s.quarantine)await oi(s.original,s.quarantine,i);else await ja(s.original)}catch(m){let c=[m];try{if((await n(s.original.path)).kind==="missing")await oi({...e,path:s.original.path},a,i)}catch(v){c.push(v)}throw AggregateError(c,`Compatibility rollback retained ambiguous generations at ${s.original.path}`)}await i(),await T(a,{force:!0})}async function of(s,i,a){let d=await A(s),e=[];try{for(let m=0;m<Xs.length;m+=1){let c=Xs[m],v=i[m],g=Y(s,c),t=Y(Ki,p,c),y;if(await U(d),await Xa(v),v.kind!=="missing"){y=Y(s,`.${c}.${cs()}.legacy`),await a(),await ls(g,y);let b=await n(y);if(!Us(v,b)){let Q=new O(`Legacy path changed during conditional migration at ${g}`);try{await oi({...b,path:g},y,a)}catch(z){throw AggregateError([Q,z],`Conditional compatibility migration rollback failed at ${g}`)}throw Q}}let x={original:v,installed:{path:g,kind:"symlink",target:t},quarantine:y,created:!1};e.push(x);try{await U(d),await a(),await Bi(t,g),x.created=!0;let b=await q(g,{bigint:!0});if(!b.isSymbolicLink())throw Error(`Installed compatibility path is not a symlink at ${g}`);x.installed={...x.installed,stats:b},await Xa(x.installed)}catch(b){let Q=[];if(!x.created){if(e.pop()!==x)throw Error(`Compatibility replacement bookkeeping changed at ${g}`)}if(!x.created&&y){let z=!1;try{let V=await n(g),Z=await n(y);if(!Us(v,Z))throw new O(`Quarantined compatibility generation changed at ${g}`);if(V.kind!=="missing")throw z=!0,await a(),await T(y,{force:!0}),new O(`Compatibility destination changed during migration at ${g}`);await oi(v,y,a)}catch(V){if(z)throw V;Q.push(V)}}if(Q.length)throw AggregateError([b,...Q],`Compatibility installation failed and rollback was incomplete at ${g}`);if(b instanceof O)throw b;if(typeof b==="object"&&b!==null&&"code"in b&&b.code==="EEXIST")throw new O(`Compatibility destination changed during migration at ${g}`);throw b}}return e}catch(m){let c=[];for(let v of e.reverse())try{await U(d),await Wi(v,a)}catch(g){c.push(g)}if(c.length)throw AggregateError([m,...c],"Compatibility setup failed and rollback was incomplete");throw m}finally{await d.handle.close()}}async function Xd(s){for(let i of Xs)try{if(await za(Y(s,i))!==Y(Ki,p,i))return!1}catch(a){if(typeof a==="object"&&a!==null&&"code"in a&&(a.code==="ENOENT"||a.code==="EINVAL"))return!1;throw a}return!0}async function Vd(s,i,a){let d=await A(s);try{await U(d),await a(),await Yd(i,`v1
`,{flag:"wx"})}finally{await d.handle.close()}}async function Lf(s,i,a,d,e=!0){await d();let m=await rs(s,!0);await Ci(s,!0);let c=Y(m,Jd);if(await od(c)){if(!await Xd(s))throw Error("Initialized review store has missing or replaced compatibility paths");let t=i===void 0,y=i??await Rf(s);if(!y)throw Error("Initialized review store is missing its current bundle");if(!e)return y;let x=await Pd(y.path,y.directory),b=await Fd(x,a);if(t&&b.state.status==="approved"){let{approvedAt:Q,reviewer:z,approvalDigest:V,...Z}=b.state;b={state:{...Z,status:"needs_revision"},comments:b.comments}}if(ji(b,x))return y;if(t)return xs(s,b,d);return x.state.status==="approved"&&b.state.status!=="approved"?Uf(s,y,b,d):xs(s,b,d)}if(i&&await Xd(s))return await Vd(m,c,d),i;let g;for(let t=0;t<10;t+=1){let y=await Cd(s),x=Y(m,p),b=await n(x),Q=await xs(s,await Fd(Od(y),a),d),z=await n(x),V=[];try{V=await of(s,y,d),await Vd(m,c,d);for(let Z of V)if(Z.quarantine)await d(),await T(Z.quarantine,{force:!0});g=Q;break}catch(Z){let W=[],I=await n(c);if(I.kind!=="missing")try{await Wi({original:{path:c,kind:"missing"},installed:I,created:!0},d)}catch(B){W.push(B)}for(let B of V.reverse())try{await Wi(B,d)}catch(j){W.push(j)}try{await Wi({original:b,installed:z,created:!0},d)}catch(B){W.push(B)}if(W.length)throw AggregateError([Z,...W],"Review-store initialization failed and rollback was incomplete");if(!(Z instanceof O))throw Z}}if(!g)throw Error("Legacy review state kept changing during migration");return await Ka(s,g,d),g}async function Ka(s,i,a){await a();let d=await rs(s,!1),e=await Ci(s,!1),[m,c]=await Promise.all([A(d),A(e)]);try{let v=await As(e);for(let t of v.filter((y)=>y.endsWith(".staging")||y!==i.id)){await U(c);let y=Y(e,t),x;try{x=await A(y)}catch(Q){if(typeof Q==="object"&&Q!==null&&"code"in Q&&Q.code==="ENOENT")continue;throw Q}let b=Y(e,`.cleanup-${cs()}`);try{await U(x),await U(c),await a(),await ls(y,b);let Q=await q(b,{bigint:!0});if(Q.dev!==x.stats.dev||Q.ino!==x.stats.ino){console.warn(`Retained rebound review bundle cleanup candidate at ${b}`);continue}let z=await x.handle.stat({bigint:!0});if(z.dev!==Q.dev||z.ino!==Q.ino){console.warn(`Retained changed review bundle cleanup candidate at ${b}`);continue}await a(),await T(b,{recursive:!0})}finally{await x.handle.close()}}let g=await As(d);for(let t of g.filter((y)=>y.startsWith("current.")&&(y.endsWith(".pointer")||y.endsWith(".invalid")||y.endsWith(".rollback"))||y.startsWith(`${Jd}.`)&&y.endsWith(".tmp"))){await U(m);let y=Y(d,t),x=await q(y,{bigint:!0}),b=Y(d,`.cleanup-${cs()}`);await a(),await ls(y,b);let Q=await q(b,{bigint:!0});if(x.dev!==Q.dev||x.ino!==Q.ino){console.warn(`Retained rebound temporary review-store path at ${b}`);continue}await a(),await T(b)}}finally{await Promise.allSettled([m.handle.close(),c.handle.close()])}}async function zd(s,i,a,d,e){await e();let m;try{m=await rs(s,!1)}catch(g){if(typeof g==="object"&&g!==null&&"code"in g&&g.code==="ENOENT")return;throw g}let c=Y(m,p),v=await n(c);if(v.kind==="missing"||!v.stats||!Ws(v.stats,d.pointer))return;try{await U(a)}catch(g){let t=await A(m);try{if(!await Ri(t,c,d,e))throw Error("Committed review pointer changed before failed transaction recovery")}catch(y){throw AggregateError([g,y],"Prior review generation was unavailable and the failed transaction could not be conditionally quarantined")}finally{await t.handle.close().catch(()=>{return})}throw AggregateError([g],"Prior review generation was unavailable; the failed transaction was quarantined without approved authority")}await Qa(s,i,a,d,e)}async function ai(s,i,a={}){s=await Qs(s);let d=await ya(s),e,m;try{await d.assertOwned();let c=await Rd(s);e=await Kd(s);let v=!1;if(c){let b=await ys(c.path,e,c.directory);await a.preflight?.(b,e),v=!0}else{let b=await Cd(s);if(!b.some((Q)=>Q.kind==="symlink")){let Q=Od(b);qa(e.plan,Q.comments),await a.preflight?.(Q,e),v=!0}}await d.assertOwned();let g=await Lf(s,c,e,d.assertOwned,a.normalize);if(!v&&a.preflight){let b=await ys(g.path,e,g.directory);await a.preflight(b,e)}if(a.cleanup!==!1)await Ka(s,g,d.assertOwned);if(a.mutating)m=await A(g.path);let t,y=(b)=>{if(t)throw Error("A review transaction cannot publish more than one current generation");t=b},x;try{x=await i(g,e,d.assertOwned,s,y)}catch(b){if(t&&m)try{await zd(s,g,m,t,d.assertCanonicalOwned)}catch(Q){throw AggregateError([b,Q],"Review transaction failed after publication and recovery was incomplete")}throw b}if(a.mutating)try{await d.assertOwned()}catch(b){if(t&&m)try{await zd(s,g,m,t,d.assertCanonicalOwned)}catch(Q){throw AggregateError([b,Q],"Review transaction lost ownership after publication and recovery was incomplete")}throw b}if(t)Wf(s,t.id);return x}finally{await m?.handle.close().catch(()=>{return}),await Ud(e),await d.release()}}async function ii(s){return ai(s,async(i,a,d,e)=>{let m=await ys(i.path,a,i.directory);if(m.state.status==="approved"&&m.state.approvalDigest!==si(a,m.state,m.comments))throw Error("Approved review identity changed while it was being read");if(m.state.status==="approved")await d(),await Ja(e,a);return{...m,generation:i.id}})}async function Ua(s,i){s=await Qs(s);let a=await ya(s),d;try{await a.assertOwned();let e=await Rd(s);if(!e)throw new gs("not_found","No coherent approved handoff is published");d=await Kd(s);let m=await ys(e.path,d,e.directory);try{Wa(d,m)}catch{throw new gs("not_found","No coherent approved handoff is published")}return await a.assertOwned(),await Ja(s,d),i==="agent-handoff.json"?m.handoffJson:m.handoffMarkdown}finally{await Ud(d),await a.release()}}function Oi(s,i,a){return{...s,...i,answers:{...s.answers,...i.answers??{}},checklist:{...s.checklist,...i.checklist??{}},unresolvedCommentIds:a.filter((d)=>d.status==="open").map((d)=>d.id)}}async function nd(s,i){if(i&&typeof i==="object"&&!Array.isArray(i)){let e=i;if(e.status==="approved"||"approvedAt"in e||"reviewer"in e||"approvalDigest"in e)throw Error("Approval status and metadata can only be set through /api/approve")}let a=Vi(i);if(a.length)throw Error(a.join(`
`));let d=i;return ai(s,async(e,m,c,v,g)=>{let t=await ys(e.path,m,e.directory),y=Oi(t.state,d,t.comments),x=t.handoffJson,b=t.handoffMarkdown;if(t.state.status==="approved"&&(d.status!==void 0||d.answers!==void 0||d.checklist!==void 0)){let{approvedAt:z,reviewer:V,approvalDigest:Z,...W}=y;y={...W,status:"needs_revision"},x=void 0,b=void 0}let Q=Js(y);if(Q.length)throw Error(Q.join(`
`));return await xs(v,{...t,state:y,handoffJson:x,handoffMarkdown:b},c,g),y},{mutating:!0})}async function Md(s,i){if(typeof i.blockId!=="string"||i.blockId.trim().length===0)throw Error("Comment blockId must be nonblank");if(typeof i.body!=="string"||i.body.trim().length===0)throw Error("Comment body must be nonblank");if(i.anchor!==void 0&&(typeof i.anchor!=="string"||i.anchor.trim().length===0))throw Error("Comment anchor must be nonblank when present");let a=(d)=>{if(!Bd(d.plan,i.blockId))throw new gs("unprocessable",`Unknown comment blockId '${i.blockId}'`)};return ai(s,async(d,e,m,c,v)=>{a(e);let g=await ys(d.path,e,d.directory),t=i.id??`c-${cs()}`;if(t.trim().length===0)throw Error("Comment id must be nonblank");let y=g.comments.find((Z)=>Z.id===t);if(y){if(y.blockId===i.blockId&&y.anchor===i.anchor&&y.body===i.body)return y;throw new gs("conflict",`Comment id '${t}' is already bound to a different payload`)}let x={id:t,blockId:i.blockId,anchor:i.anchor,body:i.body,status:"open",createdAt:new Date().toISOString()},b=[...g.comments,x],Q=Oi(g.state,{},b),z=g.handoffJson,V=g.handoffMarkdown;if(g.state.status==="approved"){let{approvedAt:Z,reviewer:W,approvalDigest:I,...B}=Q;Q={...B,status:"needs_revision"},z=void 0,V=void 0}return await xs(c,{...g,state:Q,comments:b,handoffJson:z,handoffMarkdown:V},m,v),x},{mutating:!0,preflight:(d,e)=>a(e)})}async function Nd(s,i){let a=(d)=>{if(d.comments.filter((m)=>m.id===i).length!==1)throw new gs("not_found",`Unknown or ambiguous comment id '${i}'`)};return ai(s,async(d,e,m,c,v)=>{let g=await ys(d.path,e,d.directory);if(a(g),g.comments.find((x)=>x.id===i).status==="resolved")return g.comments;let t=g.comments.map((x)=>x.id===i?{...x,status:"resolved",resolvedAt:new Date().toISOString()}:x),y=Oi(g.state,{},t);return await xs(c,{...g,state:y,comments:t},m,v),t},{mutating:!0,preflight:a})}function Zd(s,i){let a=[...s.plan.blocks,...s.canvas?.blocks??[]],d=[...i.comments.some((e)=>e.status==="open")?["AgentHandoff cannot be generated while unresolved blocking comments remain"]:[],...ma(a,i.state)];if(d.length)throw new gs("unprocessable",d.join(`
`))}async function Sd(s,i="local-reviewer"){if(i.trim().length===0)throw Error("Approval reviewer must be nonblank");return ai(s,async(a,d,e,m,c)=>{let v=d.plan,g=await ys(a.path,d,a.directory);Zd(v,g);let t=new Date().toISOString(),y=Oi(g.state,{status:"approved",approvedAt:t,reviewer:i,approvalDigest:void 0},g.comments),x={...y,approvalDigest:si(d,y,g.comments)},b=Js(x);if(b.length)throw Error(b.join(`
`));let Q=ga(v,x),z=ps(Q);if(z.length)throw Error(z.join(`
`));let V=`${JSON.stringify(Q,null,2)}
`,Z=ta(Q);Wa(d,{state:x,comments:g.comments,handoffJson:V,handoffMarkdown:Z});let W=async()=>{if(await e(),await Ja(m,d),x.approvalDigest!==si(d,x,g.comments))throw Error("Plan source or review state changed during approval")},I=await A(a.path),B;try{B=await xs(m,{state:x,comments:g.comments,handoffJson:V,handoffMarkdown:Z},e,c,W,!0)}catch(j){if(j instanceof Za&&j.restorable)try{await Qa(m,a,I,j.published,e)}catch(ms){throw AggregateError([j,ms],"Approval publication verification failed and prior authority could not be restored")}else await I.handle.close().catch(()=>{return});throw j}try{await W()}catch(j){try{await Qa(m,a,I,B,e)}catch(ms){throw AggregateError([j,ms],"Approval identity failed and prior authority could not be restored")}throw j}await I.handle.close().catch(()=>{return});try{await Ka(m,B,e)}catch(j){console.warn(`Approval committed; deferred review-store cleanup: ${j instanceof Error?j.message:String(j)}`)}return Q},{mutating:!0,normalize:!1,cleanup:!1,preflight:(a,d)=>Zd(d.plan,a)})}var Id=new Map;function Rs(s){let i=Id.get(s);if(i)return i;let a={presence:"waiting",waiterCount:0,connections:new Set};return Id.set(s,a),a}function Pf(s,i,a){for(let d of Rs(s).connections)d.send(i,a)}function oa(s,i){let a=Rs(s);if(a.presence===i)return;a.presence=i,Pf(s,"presence",{presence:i})}function Cf(s){let i=Rs(s);i.waiterCount+=1,oa(s,"listening")}function Of(s){let i=Rs(s);if(i.waiterCount=Math.max(0,i.waiterCount-1),i.waiterCount===0)oa(s,"working")}function nf(s){let i=Rs(s);if(i.waiterCount=Math.max(0,i.waiterCount-1),i.waiterCount===0)oa(s,"waiting")}function Mf(s,i){return new TextEncoder().encode(`event: ${s}
data: ${JSON.stringify(i)}

`)}function Nf(s,i,a,d){let e,m=new ReadableStream({start(c){let v=!1,g,t=Rs(s),y={send:(b,Q)=>{if(!v)c.enqueue(Mf(b,Q))},close:()=>{if(v)return;v=!0,g?.(),i.removeEventListener("abort",x),a.removeEventListener("abort",x),t.connections.delete(y);try{c.close()}catch{}}},x=()=>y.close();e=y.close,d(y.close),t.connections.add(y),i.addEventListener("abort",x,{once:!0}),a.addEventListener("abort",x,{once:!0}),y.send("presence",{presence:t.presence});try{g=Wd(s,({generation:b})=>{y.send("review-update",{generation:b})})}catch{y.close()}},cancel(){e?.()}});return new Response(m,{headers:{"cache-control":"no-cache, no-store",connection:"keep-alive","content-type":"text/event-stream; charset=utf-8"}})}async function Ra(s){try{return await s.json()}catch{throw new Response("Invalid JSON",{status:400})}}function Dd(s){if(!s||typeof s!=="object"||Array.isArray(s))throw new Response("JSON body must be an object",{status:400});return s}function Sf(s,i,a){let d=s.headers.get("origin");if(d!==null){let e;try{e=new URL(d)}catch{}let m=i.hostname==="localhost"||i.hostname==="127.0.0.1"||i.hostname==="[::1]";if(!e||d==="null"||e.origin!==i.origin||!m||Number(i.port)!==a)throw new Response("Foreign mutation origin",{status:403})}if(s.headers.get("content-type")?.split(";",1)[0].trim().toLowerCase()!=="application/json")throw new Response("Mutating API requests require application/json",{status:415})}function $f(s,i){let a=Object.keys(s);if("resolveId"in s){if(a.length!==1||typeof s.resolveId!=="string"||s.resolveId.trim().length===0)throw new Response("Comment resolution body must be exactly { resolveId: nonblank string }",{status:400});return{mode:"resolve",resolveId:s.resolveId}}let d=s.id??i??void 0;if(a.some((e)=>e!=="id"&&e!=="blockId"&&e!=="anchor"&&e!=="body")||d!==void 0&&(typeof d!=="string"||d.trim().length===0)||i!==null&&(i.trim().length===0||d!==i)||typeof s.blockId!=="string"||s.blockId.trim().length===0||typeof s.body!=="string"||s.body.trim().length===0||s.anchor!==void 0&&(typeof s.anchor!=="string"||s.anchor.trim().length===0))throw new Response("Comment body must contain matching optional id/idempotency-key plus nonblank blockId, body, and optional anchor",{status:400});return{mode:"add",id:d,blockId:s.blockId,anchor:s.anchor,body:s.body}}async function kd(s,i=7374,a){s=await Bf(s),await zi(s),a?.throwIfAborted();let d=new AbortController,e=new Set,m=Bun.serve({port:i,async fetch(y){let x=new URL(y.url);try{if(x.pathname.startsWith("/assets/")){let b=x.pathname.slice(8);if(!$a(b))return new Response("Not found",{status:404});return new Response(await $d(Ed(s,"dist","assets",b)),{headers:{"cache-control":"public, max-age=31536000, immutable","content-type":"font/woff2"}})}if(x.pathname==="/"||x.pathname==="/index.html"||x.pathname==="/static-export.html"){let b=x.pathname==="/static-export.html"?"static-export.html":"index.html";return new Response(await $d(Ed(s,"dist",b),"utf8"),{headers:{"content-type":"text/html; charset=utf-8"}})}if(x.pathname==="/plan-state.json"){let b=await ii(s);return Response.json(b.state,{headers:{"x-muse-review-generation":b.generation}})}if(x.pathname==="/comments.json"){let b=await ii(s);return Response.json(b.comments,{headers:{"x-muse-review-generation":b.generation}})}if(x.pathname==="/agent-handoff.json")return new Response(await Ua(s,"agent-handoff.json"),{headers:{"content-type":"application/json; charset=utf-8"}});if(x.pathname==="/agent-handoff.md")return new Response(await Ua(s,"agent-handoff.md"),{headers:{"content-type":"text/markdown; charset=utf-8"}});if(x.pathname==="/api/events"&&y.method==="GET")return Nf(s,y.signal,d.signal,(b)=>e.add(b));if(x.pathname==="/api/wait"&&y.method==="GET"){let b=x.searchParams.get("since")??"";try{let Q=await jd(s,b,{signal:AbortSignal.any([y.signal,d.signal]),onParked:()=>Cf(s),onDelivered:()=>Of(s),onCancelled:()=>nf(s)});return Response.json({generation:Q.generation,state:Q.state,comments:Q.comments},{headers:{"x-muse-review-generation":Q.generation}})}catch(Q){if(Q instanceof Pi)return new Response(Q.message,{status:503});if(y.signal.aborted||d.signal.aborted)return new Response(null,{status:204});throw Q}}if(x.pathname.startsWith("/api/")&&y.method==="POST")Sf(y,x,m.port??i);if(x.pathname==="/api/state"&&y.method==="POST"){let b=await Ra(y);if(b&&typeof b==="object"&&!Array.isArray(b)){let z=b;if(z.status==="approved"||"approvedAt"in z||"reviewer"in z||"approvalDigest"in z)throw new Response("Approval status and metadata can only be set through /api/approve",{status:409})}let Q=Vi(b);if(Q.length)throw new Response(Q.join(`
`),{status:400});return Response.json(await nd(s,b))}if(x.pathname==="/api/comments"&&y.method==="POST"){let b=$f(Dd(await Ra(y)),y.headers.get("idempotency-key"));if(b.mode==="resolve")return Response.json(await Nd(s,b.resolveId));return Response.json(await Md(s,{id:b.id,blockId:b.blockId,anchor:b.anchor,body:b.body}))}if(x.pathname==="/api/approve"&&y.method==="POST"){let b=Dd(await Ra(y)),Q=Object.keys(b),z=b.reviewer;if(Q.some((V)=>V!=="reviewer")||Q.length>1||z!==void 0&&(typeof z!=="string"||z.trim().length===0))return new Response("Approval body must be exactly {} or { reviewer: nonblank string }",{status:400});return Response.json(await Sd(s,z))}return new Response("Not found",{status:404})}catch(b){if(b instanceof Response)return b;if(b instanceof gs){let Q=b.failure==="not_found"?404:b.failure==="conflict"?409:422;return new Response(b.message,{status:Q})}return new Response(b instanceof Error?b.message:String(b),{status:500})}}}),c=m.stop.bind(m),v=!1,g=(y=!0)=>{if(!v){v=!0,d.abort();for(let x of e)x();e.clear()}return c(y)};m.stop=g;let t=()=>g(!0);if(a?.addEventListener("abort",t,{once:!0}),a?.aborted)t(),a.throwIfAborted();return m}function La(){throw Error("Usage: bun runtime.mjs <render|serve> <plan-dir> [port]")}var[Ba,Pa,Ef]=process.argv.slice(2);if(!Ba||!Pa)La();if(Ba==="render"){let s=await zi(Pa);console.log(`Rendered ${s.indexPath}`),console.log(`Static ${s.staticExportPath}`)}else if(Ba==="serve"){let s=Number(Ef??7374);if(!Number.isInteger(s)||s<0||s>65535)La();let i=await kd(Pa,s);console.log(`muse plan review: http://localhost:${i.port}/`)}else La();
