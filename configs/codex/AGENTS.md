...
Define how Codex discovers and uses the canonical `do-muse` skill from `plugins/Muse/`.
...
- Install the skill under `~/.agents/skills/do-muse`; Codex scans that user-level Agent Skills directory and follows symlinked skill folders.
...
Ask `Use do-muse to <task>`. Codex may also select `do-muse` for a natural-language request for a diagram, visual explainer, diff review, plan review, slide deck, or complex table.
